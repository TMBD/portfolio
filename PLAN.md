...
- 2026-07-16 — Polish fixes — `package.json` metadata (Thierno DIALLO / thierno.dev@gmail.com / v1.0.0 / homepage); **skill logos → full-color** Iconify `logos` set (`@iconify-json/logos`); removed footer "Built with…"; **theme-aware text selection** (subtle accent tint, no forced white text); **header utility cluster → `lg:flex`** so it no longer duplicates in the mobile menu at medium widths; set `astro.config` `site` to `https://thierno.dev`.
- 2026-07-16 — Social icons colored (footer + About): LinkedIn full-colour logo, GitHub theme-adaptive monochrome, email accent-tinted, + hover scale. Phase 8 prep: `.nvmrc` (Node 20), static output (no adapter needed). Awaiting user: push to GitHub → connect Cloudflare Pages → custom domains (thierno.dev primary, thiernodiallo.dev redirect).
